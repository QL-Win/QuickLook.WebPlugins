...
export class ChmFile {
  ...
  private async init(): Promise<void> {
    ...
    // Store language id for downstream decoding hints
    this.langId = itsf.langId;
  ...
  /** Return the CHM language id (if present) */
  getLanguageId(): number | null { return this.langId; }
  ...
  private async decompressOneBlock(blockIdx: number): Promise<void> {
    ...
    let res = lzxDecompress(this.lzxState!, cbuf, ubuf, Number(len), blockLen);
    if (res !== DECR_OK) {
      // Try a single reset+retry — some archives may require re-syncing the state
      console.warn(`LZX decompression error code ${res} for block ${blockIdx}, retrying after reset`);
      this.lzxState!.reset();
      res = lzxDecompress(this.lzxState!, cbuf, ubuf, Number(len), blockLen);
      if (res !== DECR_OK) {
        // Provide a small hex snippet for debugging
        const sample = Array.from(cbuf.subarray(0, Math.min(32, cbuf.length))).map(b => b.toString(16).padStart(2, '0')).join(' ');
        throw new Error(`LZX decompression failed for block ${blockIdx}: code ${res}. firstBytes=${sample}`);
      }
    }

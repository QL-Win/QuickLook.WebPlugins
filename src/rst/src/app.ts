/**
 * rst — browser-based reStructuredText viewer
 *
 * Entry point. Handles file selection/drop, converts with rst-compiler,
 * builds the TOC sidebar, and drives fragment navigation.
 */

import { convertRst, decodeUtf8 } from './lib/convert.js';
...
const pluginMode =
  Boolean((window as unknown as { __RST_PLUGIN?: boolean }).__RST_PLUGIN) ||
...
async function navigate(fragmentOrHref: string, pushHistory = true): Promise<void> {
  ...
  if (fragment) {
    ...
    // Top of document
    const wrapper = contentHtml.querySelector('.rst-host-wrapper');
    const host = wrapper?.shadowRoot?.querySelector('.rst-host');
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  if (event.data.type === 'rst-navigate') {
    const href = String(event.data.href);
    navigate(href);
    return;
  }
  if (event.data.type === 'open-rst') {
...
async function handleHostOpenRequest(payload: unknown) {
  ...
  if (p.base64) {
    ...
    await openRstBuffer(buf, String(p.name || 'file.rst'));
    return;
  }
  if (p.text != null || p.lines != null) {
    let text: string;
    if (Array.isArray(p.lines)) {
      text = p.lines.map(String).join('\n');
    } else if (Array.isArray(p.text)) {
      // Some hosts accidentally JSON-encode lines as a string array
      text = p.text.map(String).join('\n');
    } else {
      text = String(p.text ?? '');
    }
    await openRstText(text, String(p.name || 'file.rst'));
    return;
  }
  if (p.url) {
    await openRstFromUrl(String(p.url), p.name ? String(p.name) : undefined);
    return;
  }
  if (p.path) {
    await openRstFromUrl(String(p.path), p.name ? String(p.name) : undefined);
    return;
  }
  throw new Error('Unsupported payload for open-rst');
}
...
async function openRstFromUrl(url: string, name?: string) {
  setStatus('Fetching reStructuredText…');
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Fetch failed: ${res.status} ${res.statusText}`);
  const ab = await res.arrayBuffer();
  await openRstBuffer(ab, name || extractNameFromUrl(url));
}
...
// ─── Open reStructuredText ────────────────────────────────────────────────────

async function openRstBuffer(buffer: ArrayBuffer, name: string): Promise<void> {
  const text = decodeUtf8(buffer);
  await openRstText(text, name);
}

async function openRstText(source: string, name: string): Promise<void> {
  setStatus('Converting reStructuredText…');
  ...
  try {
    const converted = await convertRst(source);
    currentDoc = converted;

    if (converted.title) {
      titleEl.textContent = converted.title;
      titleEl.hidden = false;
      document.title = `rst — ${converted.title}`;
    } else {
      titleEl.textContent = name;
      titleEl.hidden = false;
      document.title = `rst — ${name}`;
    }
    ...
    renderHtmlToDiv(converted.html, contentHtml, getResolvedTheme(), converted.header);
...
function readFile(file: File): void {
  ...
  reader.onload = () => openRstBuffer(reader.result as ArrayBuffer, file.name);
...
if (pluginMode) {
  ...
  (async () => {
    const rstParam = urlParams.get('rst') || urlParams.get('rstUrl') || urlParams.get('url');
    const rstBase64 = urlParams.get('rstBase64') || urlParams.get('base64');
    const name = urlParams.get('name') || undefined;
    try {
      if (rstBase64) {
        const buf = base64ToArrayBuffer(rstBase64);
        await openRstBuffer(buf, name || 'file.rst');
      } else if (rstParam) {
        await openRstFromUrl(rstParam, name);
      } else {
        setStatus('Waiting for host to provide reStructuredText…');
      }
    } catch (err) {
